...
<div class="weui-tab">
	...
	<div class="weui-grids">
		...
		@else
			@foreach($list as $li)	
		  	<a href="/ApplyCard/detail/{{$info['merchant']}}/{{$info['ident']}}/{{$li->id}}?showprice={{$showprice}}" class="weui-grid js_grid">
		    	<div class="weui-grid__icon"> <img src="{{ $li->getIcon() }}"> </div>
		    	<p class="weui-grid__label bold"> {{ $li->title }} </p>
		    	<p class="weui-grid__label"> <code>{{ $li->getFirstLigHeight() }}</code> </p>
		    	<p class="weui-grid__label price"> {{ $li->getSecendLigHeight() }}</p>
		    	@if($li->recommand == '1')
		    	<p class="recommand"> 推荐 </p>
		    	@endif
		  	</a>
